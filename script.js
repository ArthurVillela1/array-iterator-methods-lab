...
const checkage = inventors.filter((inventor) =>{
...
inventorNamedAda = inventors.find((names) => {
    return names.first === 'Ada'
})
...
commentById = comments.find((comObj) => {
    return comObj.id === 823423
});
...
idx = comments.findIndex((comObj) => {
    return comObj.id === 123523
...
// Enter your solution code here:
totalYearsLived = inventors.reduce((p,c) => {
    lifespan = c.passed - c.year
    return lifespan + p
}, 0)
